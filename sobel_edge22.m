function [ org,fig1,fig2,fig3 ] = sobel_edge22( input )
...
rf=imnoise(rf,'salt & pepper',0.01);
...
for i=2:h-2 
    for j=2:w-2
sumtotal(i,j)=(sum(mpp(i-1,j-1,:))+sum(mpp(i-1,j,:))+sum(mpp(i-1,j+1,:))+sum(mpp(i,j-1,:))+sum(mpp(i,j+1,:))+sum(mpp(i+1,j-1,:))+sum(mpp(i+1,j,:))+sum(mpp(i+1,j+1,:)))*.5^sum(mpp(i,j,:));
    end
end
...
for i=2:h-2 
    for j=2:w-2
     ...
     rm(i,j,:)=(rm(i+1,j,:) + rm(i+1,j-1,:) + rm(i+1,j+1,:) + rm(i,j+1,:) + rm(i,j-1,:) + rm(i-1,j,:) + rm(i-1,j-1,:) + rm(i-1,j+1,:) + rm(i,j,:))/9;
          ...
          if sumtotal(i,j)<=(st-sc*2) && sumtotal(i,j)>(st-sc*3)
     ...
     rm(i,j,:)=(rm(i+1,j,:) + rm(i+1,j-1,:) + rm(i+1,j+1,:) + rm(i,j+1,:) + rm(i,j-1,:) + rm(i-1,j,:) + rm(i-1,j-1,:) + rm(i-1,j+1,:) + rm(i,j,:)*2)/10;
           ...
           if sumtotal(i,j)<=(st-sc*3) && sumtotal(i,j)>(st-sc*4)
     ...
     rm(i,j,:)=(rm(i+1,j,:) + rm(i+1,j-1,:) + rm(i+1,j+1,:) + rm(i,j+1,:) + rm(i,j-1,:) + rm(i-1,j,:) + rm(i-1,j-1,:) + rm(i-1,j+1,:) + rm(i,j,:)*3)/11;
            ...
            if sumtotal(i,j)<=(st-sc*4) && sumtotal(i,j)>(st-sc*5)
