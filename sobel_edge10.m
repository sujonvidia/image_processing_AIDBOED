function [ org,fig1,fig2,fig3 ] = sobel_edge10( input )
...
m1=max(mp(:));
  ...
  mp(i,j,2)=mp(i,j,2)/m1;
  mp(i,j,3)=mp(i,j,3)/m1;
      ...
      mpp(i,j,2)=(1-mp(i,j,2));
       mpp(i,j,3)=(1-mp(i,j,3));
        ...
        if mv<=3 && mv>2.9
           ...
           if  ( sum(mpp(i+1,j,:)) && sum(mpp(i+1,j-1,:)) && sum(mpp(i+1,j+1,:)) && sum(mpp(i,j+1,:)) && sum(mpp(i,j-1,:)) && sum(mpp(i-1,j,:)) && sum(mpp(i-1,j-1,:)) && sum(mpp(i-1,j+1,:)) ) < 3
               if  ( sum(mpp(i+1,j,:)) && sum(mpp(i+1,j-1,:)) && sum(mpp(i+1,j+1,:)) && sum(mpp(i,j+1,:)) && sum(mpp(i,j-1,:)) && sum(mpp(i-1,j,:)) && sum(mpp(i-1,j-1,:)) && sum(mpp(i-1,j+1,:)) )>1.8
...
fig3=figure('name','Noise Remove after Applying Median');imshow(mp);
