function [ org,fig1,fig2,fig3 ] = sobel_edge28( input )
...
for i=2:h-2 
    for j=2:w-2
sumtotal(i,j)=(sum(mpp(i-1,j-1,:))+sum(mpp(i-1,j,:))+sum(mpp(i-1,j+1,:))+sum(mpp(i,j-1,:))+sum(mpp(i,j+1,:))+sum(mpp(i+1,j-1,:))+sum(mpp(i+1,j,:))+sum(mpp(i+1,j+1,:)))*.5^sm(i,j)*3;
    end
end
...
org=figure('name','Original Picture');imshow(rm);
